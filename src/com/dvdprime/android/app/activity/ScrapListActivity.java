...
public class ScrapListActivity extends BaseActivity 
{
				...
				DialogBuilder.createConfirmDialog(activity, 
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(final DialogInterface dialog, final int which) {
	                    	dialog.dismiss();
	        				Intent intent = new Intent(ScrapListActivity.this, AccountSettingActivity.class);
	        				startActivity(intent);
	        				finish();
						}
				}).show();
		...
		if (event.getData() != null) {
			switch (event.getType()) {
				...
				case DataEvent.SCRAP_LIST:
					...
					if (dba.getScrapList(Const.TYPE_SCRAP) != null) {
						adapter = new ScrapListAdapter(this,
											R.layout.article_list_item, 
											dba.getScrapList(Const.TYPE_SCRAP));
						
						paginator = new ScrapListPaginator();
						paginator.setType(Const.TYPE_SCRAP);
	...
	private void getScrapList() {
		if (SystemUtil.isNetworkAvailable(activity)) {
			...
			sm.addConsumer(this, DataEvent.SCRAP_LIST);
			sm.list(Const.SCRAP_LIST_URL);
