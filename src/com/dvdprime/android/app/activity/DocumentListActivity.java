...
public class DocumentListActivity extends BaseActivity 
{
    ...
    protected void onCreate(Bundle icicle)
    {
        ...
        // 제목 설정
        setTitle(R.string.list_document);
				...
				DialogBuilder.createConfirmDialog(activity, 
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(final DialogInterface dialog, final int which) {
	                    	dialog.dismiss();
	        				Intent intent = new Intent(DocumentListActivity.this, AccountSettingActivity.class);
	        				startActivity(intent);
	        				finish();
						}
				}).show();
		...
		if (event.getData() != null) {
			switch (event.getType()) {
				...
				case DataEvent.DOCUMENT_LIST:
					...
					if (dba.getScrapList(Const.TYPE_DOC) != null) {
						adapter = new ScrapListAdapter(this,
											R.layout.article_list_item, 
											dba.getScrapList(Const.TYPE_DOC));
						
						paginator = new ScrapListPaginator();
						paginator.setType(Const.TYPE_DOC);
	...
	private void getDocumentList() {
		if (SystemUtil.isNetworkAvailable(activity)) {
			...
			sm.addConsumer(this, DataEvent.DOCUMENT_LIST);
			sm.documentList(Const.MY_ARTICLE_URL);
