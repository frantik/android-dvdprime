...
public class ScrapRetrieveThread extends Thread {
	...
	public ScrapRetrieveThread(String threadName, InputStream is, boolean isMore) {
		super(threadName);
	...
	public void run() {
		...
		try {
			...
			// article list header amount 8,000 characters
			br.skip(8000);
    			...
    			if(buf == null) break;
    				...
    				if (isWritable) {
    					if (iCount==0 && buf.contains("left")) {
	    					...
	    					values.put(DpDB.Scrap.TITLE, buf.substring(lstIdx+4, buf.indexOf("</a>")));
    					} else if (iCount==2 && buf.contains("center")) {
    						iCount++;
    						fstIdx = buf.indexOf('>')+1;
    						lstIdx = buf.lastIndexOf('<');
    						values.put(DpDB.Scrap.USER_NAME, buf.substring(fstIdx, lstIdx));
    						...
    						iCount++;
    						fstIdx = buf.lastIndexOf('\"')+2;
    						lstIdx = buf.lastIndexOf("</td>");
	    					values.put(DpDB.Scrap.RCMD, buf.substring(fstIdx, lstIdx));
    					} else if (iCount==5 && buf.contains("center")) {
    						iCount = 0;
    						fstIdx = buf.lastIndexOf('\"')+2;
    						lstIdx = buf.lastIndexOf("</td>");
	    					values.put(DpDB.Scrap.RCNT, buf.substring(fstIdx, lstIdx));
    				...
    				if (isWritable && buf.contains("dot_prepre.gif")) {
    					...
    					if (StringUtil.isNotBlank(tmpStr)) {
	    					values.put(DpDB.Scrap.URL, Const.SCRAP_LIST_URL + tmpStr);
	    					dbAdapter.insertScrap(values);
    					}
    				}
