...
public class CommentRetrieveThread extends Thread {
	...
	public CommentRetrieveThread(String threadName, InputStream is, boolean isMore) {
		super(threadName);
	...
	public void run() {
		...
		try {
			...
			// article list header amount 6,000 characters
			br.skip(5000);
    			...
    			if(buf == null) break;
    				...
    				if (isWritable) {
    					if (iCount==0 && buf.contains("tr height")) {
    						...
    						values.put(DpDB.Scrap.URL, 
    								StringUtil.substringBefore(buf.substring(fstIdx, lstIdx), "#"));
        					values.put(DpDB.Scrap.RCMD, 
        							StringUtil.substringAfter(
        									StringUtil.substringAfter(buf.substring(fstIdx, lstIdx), "#"),
        									"_"));
    				...
    				if (isWritable && buf.contains("dot_prepre.gif")) {
    					...
    					if (StringUtil.isNotBlank(tmpStr)) {
	    					values.put(DpDB.Scrap.URL, Const.MY_COMMENT_URL + tmpStr);
	    					dbAdapter.insertScrap(values);
    					}
    				}
