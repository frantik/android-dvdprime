...
public class MessageRetrieveMoreThread extends Thread {
	...
	public MessageRetrieveMoreThread(String threadName, InputStream is) {
		super(threadName);
    			...
    			if(buf == null) break;
    				...
    				if (isWritable) {
    					if (iCount==0 && buf.contains("\"center\"")) {
    						...
    						// 게시물이 존재하지 않을 경우에만 목록에 추가
    						if (!dbAdapter.isExistArticle(
    								values.getAsString(DpDB.Article.ATC_NO), 
    								values.getAsString(DpDB.Article.ATC_TITLE))) {
        						// 필터 적용
        						if (filterIds != null 
        								&& filterIds.length > 0
    				...
    				if (isWritable && buf.contains("dot_prepre.gif")) {
    					...
    					if (tmpStr.contains("href=")) {
    						...
    						values.put(DpDB.Article.ATC_URL, "");
    					}
    					
    					if (values.get(DpDB.Article.ATC_URL).equals(""))
        					values.put(DpDB.Article.ATC_TITLE, mContext.getResources().getString(R.string.bbs_progressbar_end));
    					else
